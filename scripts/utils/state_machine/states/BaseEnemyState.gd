...
func check_detect_player():
	if char_body.detected_player:
		if char_body.is_seeker and not char_body.enemy_state_machine.state.name == "EnemyChaseState":
			finished.emit("EnemyChaseState")
		#elif not char_body.enemy_state_machine.state.name == "EnemyApproachState":
